...
async fn persist_book_series_contribution(ctx: &TestFixture, book_id: &str) {
...
async fn runtime_refresh_series_metadata_applies_epub_from_book_provider_patch() {
    ...
    pool.close().await;
    persist_book_series_contribution(&ctx, "book-1").await;
...
async fn runtime_refresh_series_metadata_ignores_deleted_books_from_book_providers() {
    ...
    pool.close().await;
    persist_book_series_contribution(&ctx, "book-1").await;
    persist_book_series_contribution(&ctx, "book-deleted-series-provider").await;
