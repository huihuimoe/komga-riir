...
async fn router_actuator_metric_detail_exposes_datasource_tags_for_admin() {
    ...
    let _main_pool_one = komga_infrastructure_base::connect_shared_pool(&ctx.paths().main_db, 1)
        .await
        .expect("main shared sqlx pool with max=1 should open");
    let _main_pool_two = komga_infrastructure_base::connect_shared_pool(&ctx.paths().main_db, 2)
        .await
        .expect("main shared sqlx pool with max=2 should open");
    let _tasks_pool_one = komga_infrastructure_base::connect_shared_pool(&ctx.paths().tasks_db, 1)
        .await
...
async fn router_actuator_health_aggregates_down_when_database_file_is_missing() {
    ...
    komga_infrastructure_base::file_io::remove_file_after_release(&ctx.paths().tasks_db)
