...
pub(crate) async fn execute_repair_extension(
    ...
    repair_extension(runtime.media_library(), book_id).await?;

    Ok(TaskExecutionOutcome::completed())
}

pub(crate) async fn execute_find_books_to_convert(
    runtime: &JobRuntime<'_>,
    library_id: &str,
    priority: i32,
) -> Result<TaskExecutionOutcome, TaskProcessingError> {
    ...
    let books = find_books_to_convert(runtime.media_library(), library_id).await?;
...
pub(crate) async fn execute_convert_book(
    runtime: &JobRuntime<'_>,
    book_id: &str,
) -> Result<TaskExecutionOutcome, TaskProcessingError> {
    if !runtime.database().owns_main_database() {
